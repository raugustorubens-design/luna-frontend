...
- ~~"Guardian" (Storage Manager) entre Memory Engine e o banco não existe~~ — **resolvido no Guardian MVP-01** (repositório `raugustorubens-design/luna-api`). Memory Engine agora depende só de `GuardianContract`; ver LUNA_CONTEXT.md §15.
- **Novo, registrado no Guardian MVP-01**: o Guardian real (`luna-api`) não está implantado — Memory Engine usa `guardian-local-adapter.ts`, uma implementação local transitória do mesmo contrato. Corte para um cliente HTTP real fica para quando o deploy do Guardian acontecer (fora de escopo do MVP-01, que era só "preparar a arquitetura").
...
- ~~`/chat` do Forge tem contrato incompatível com este cliente e `/context` não existe no backend configurado~~ — **era um diagnóstico errado, não um gap real**: o backend em `NEXT_PUBLIC_LUNA_API_BASE_URL` (`strong-celebration-production.up.railway.app`) sempre foi o deploy Railway do próprio Cognitive Engine (monorepo `luna`), só rodando um `railway.json` quebrado que o fazia servir um protótipo Vite órfão em vez do backend real — corrigido em LUNA-001 (monorepo `luna`, commits `d17a668`/`0651068`, `luna_context/LUNA_CONTEXT.md` §16). Nenhuma URL mudou aqui; só a documentação em `lib/forge/api-client.ts` e `DEPLOY.md`, que atribuía esse backend a um serviço `luna-guardian` inexistente nesse papel. Ver handoff `BUILDER_HANDOFF_chat_redirect.md`.
